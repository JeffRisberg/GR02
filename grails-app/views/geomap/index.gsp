	...
	<body>		
      ...
      <g:each in="${overlays}" var="overlay">     
        <g:if test="${overlay.shown && overlay.kmlFileUrl}">
          ...
          google.maps.KmlLayer('${overlay.kmlFileUrl}', kmlLayerOptions);
        ...
        <g:if test="${overlay.shown && overlay.kmlFileUrl == null}">
          ...
          <g:each in="${overlay.records}" var="record">    
            <g:if test="${record.name && record.address?.latitude && record.address?.longitude}">        
          ...
          $.each(markers, function(index, marker) {            
            ...
            $("<li></li>")
            .text(marker.title)
            .bind('click', function(event) { displayPoint(marker); } )
      ...
      function handleChangeCenterOrZoom(event) {    
       ...
       $.ajax({type: 'POST',
          url: '${createLink(controller: "geomap", action: "handleChangeCenterOrZoom")}',
          data: { centerLat: center.lat(), centerLng: center.lng(), zoom: zoom, mapTypeId: mapTypeId }
         });
      }
