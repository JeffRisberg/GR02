...
class ResourceController {
	...
	def list = {
		...
		dfp = new DisplayFilterPojo(name: 'resourceType', label: 'Resource Type:', type: 'Select',
		values: ResourceType.list())
		filters.add(dfp)
		
		// Keep these values so we can rerender while maintaining filter value settings
		flash.resourceType = params.resourceType
		
		def criteria = Resource.createCriteria()
		def query = {
			and {
				if (params.resourceType) {
					def selectedResourceType = ResourceType.get(Integer.parseInt(params.resourceType))
					if (selectedResourceType) {
						eq('type', selectedResourceType)
					}
				}
			}
		}
		...
		[filters: filters, resourceInstanceList: results, resourceInstanceTotal: results.totalCount]
	}
